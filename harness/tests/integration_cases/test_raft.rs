...
use raft::eraftpb::*;
use raft::storage::{last_index_if_initialized, MemStorage};
use raft::*;
use slog::Logger;
...
fn test_progress_leader() {
    ...
    for i in 0..5 {
        ...
        assert_eq!(matched, i + 1 + last_index_if_initialized(2));
...
fn test_leader_election_overwrite_newer_logs_with_config(pre_vote: bool, l: &Logger) {
    ...
    // Now all nodes agree on a log entry with term 1 at index 5 (and term 3 at index 6).
    let last_idx = last_index_if_initialized(5) as usize;
    for (id, sm) in &network.peers {
        ...
        assert_eq!(
            entries[last_idx].term, 1,
            "node {}: term at index 5 == {}, want 1",
            id, entries[last_idx].term
        );
        assert_eq!(
            entries[last_idx + 1].term,
            3,
            "node {}: term at index 6 == {}, want 3",
            id,
            entries[last_idx + 1].term
        );
    }
...
fn test_single_node_commit() {
    ...
    // New created peer's raft log starts at 1, which is a configure change entry.
    assert_eq!(tt.peers[&1].raft_log.first_index(), 1);
...
fn test_cannot_commit_without_new_term_entry() {
    ...
    let base_idx = last_index_if_initialized(5);
    assert_eq!(tt.peers[&1].raft_log.committed, base_idx);
    tt.send(vec![new_message(1, 1, MessageType::MsgHup, 0)]);
    assert_eq!(tt.peers[&1].raft_log.committed, base_idx + 1); // Empty entry of the term.
    ...
    assert_eq!(tt.peers[&1].raft_log.committed, base_idx + 1);
    ...
    assert_eq!(tt.peers[&2].raft_log.committed, base_idx + 1);
    ...
    assert_eq!(tt.peers[&2].raft_log.committed, base_idx + 5);
}
...
fn test_commit_without_new_term_entry() {
    ...
    assert_eq!(tt.peers[&1].raft_log.committed, base_idx + 4);
}
...
fn test_dueling_candidates() {
    ...
    let raft_logs = vec![
        ...
        (base_idx + 1, base_idx, base_idx + 1),
        (base_idx + 1, base_idx, base_idx + 1),
...
fn test_candidate_concede() {
    ...
    let base_idx = last_index_if_initialized(3); // Initialized with 3 peers.
    for p in tt.peers.values() {
        assert_eq!(p.raft_log.committed, base_idx + 2); // All raft logs are committed.
        assert_eq!(p.raft_log.applied, base_idx); // Raft logs are based on a snapshot with index 1.
        assert_eq!(p.raft_log.last_index(), base_idx + 2);
    }
}
...
fn test_old_messages() {
    ...
    for p in tt.peers.values() {
        ...
        assert_eq!(raft.raft_log.committed, base_idx + 4);
        assert_eq!(raft.raft_log.applied, base_idx);
        assert_eq!(raft.raft_log.last_index(), base_idx + 4);
    }
}
...
fn test_proposal() {
    ...
    for (j, (mut nw, success)) in tests.drain(..).enumerate() {
        ...
        let base_idx = last_index_if_initialized(nw.peers.len());
        let want_log = if success {
            (base_idx + 2, base_idx, base_idx + 2)
        } else {
            (base_idx, base_idx, base_idx)
        };
...
fn test_msg_append_response_wait_reset() {
    ...
    m.index = base_idx + 1;
    sm.step(m).expect("");
    assert_eq!(sm.raft_log.committed, base_idx + 1);
    ...
    assert_eq!(msgs[0].entries[0].index, base_idx + 2);
    ...
    assert_eq!(msgs[0].entries[0].index, base_idx + 1);
}
...
fn test_all_server_stepdown() {
    let l = default_logger();
    let last_index = last_index_if_initialized(3);
    let mut tests = vec![
        // state, want_state, term, last_index, entry count.
        (StateRole::Follower, StateRole::Follower, 3, last_index),
        (StateRole::PreCandidate, StateRole::Follower, 3, last_index),
        (StateRole::Candidate, StateRole::Follower, 3, last_index),
        (
            StateRole::Leader,
            StateRole::Follower,
            3,
            last_index + 1, /* for become leader */
        ),
    ];
...
fn test_read_only_option_safe() {
    ...
    let mut tests = vec![
        // id, proposals, base_idx + empty + proposals, context.
        (1, 10, base_idx + 11, vec!["ctx1", "ctx11"], false),
        (2, 10, base_idx + 21, vec!["ctx2", "ctx22"], false),
        (3, 10, base_idx + 31, vec!["ctx3", "ctx33"], false),
        (1, 10, base_idx + 41, vec!["ctx4", "ctx44"], true),
        (2, 10, base_idx + 51, vec!["ctx5", "ctx55"], true),
        (3, 10, base_idx + 61, vec!["ctx6", "ctx66"], true),
    ];
...
fn test_read_only_with_learner() {
    ...
    let mut nt = Network::new(vec![Some(a), Some(b)], &l);
    let base_idx = last_index_if_initialized(2); // 1 votere and 1 learner.
    ...
    let mut tests = vec![
        // id, proposals, base_idx + empty_entry + proposals, context.
        (1, 10, base_idx + 11, "ctx1"),
        (2, 10, base_idx + 21, "ctx2"),
        (1, 10, base_idx + 31, "ctx3"),
        (2, 10, base_idx + 41, "ctx4"),
    ];
...
fn test_read_only_option_lease() {
    ...
    let mut tests = vec![
        (1, 10, base_idx + 11, "ctx1"),
        (2, 10, base_idx + 21, "ctx2"),
        (3, 10, base_idx + 31, "ctx3"),
        (1, 10, base_idx + 41, "ctx4"),
        (2, 10, base_idx + 51, "ctx5"),
        (3, 10, base_idx + 61, "ctx6"),
    ];
...
fn test_read_only_option_lease_without_check_quorum() {
    ...
    let rs = &read_states[0];
    assert_eq!(rs.index, last_index_if_initialized(3) + 1);
    let vec_ctx = ctx.as_bytes().to_vec();
...
fn test_read_only_for_new_leader() {
    ...
    let base_idx = last_index_if_initialized(3);
    let mut peers = vec![];
    for (id, mut committed, mut applied, mut compact_index) in node_configs {
        let storage = MemStorage::new_with_conf_state((vec![1, 2, 3], vec![]));
        let base_idx = last_index_if_initialized(3);
        committed += base_idx;
        applied += base_idx;
        compact_index += base_idx;

        let mut cfg = new_test_config(id, 10, heartbeat_ticks);
        cfg.applied = applied;
        let entries = vec![empty_entry(1, base_idx + 1), empty_entry(1, base_idx + 2)];
    ...
    nt.send(vec![new_message(1, 1, MessageType::MsgPropose, 1)]);
    assert_eq!(nt.peers[&1].raft_log.committed, base_idx + 4);
    assert_eq!(
        nt.peers[&1]
            .raft_log
...
fn test_leader_append_response() {
    let l = default_logger();
    let base_idx = last_index_if_initialized(3);
    // Initial progress: match = 0, next = base_idx + 2 + 1 on followers.
    let mut tests = vec![
        // Stale resp; no replies.
        (base_idx + 3, true, 0, base_idx + 3, 0, 0, 0),
        ...
        tests.drain(..).enumerate()
    {
        ...
        let next_idx = store.last_index().unwrap() + 1;
        let ents: Vec<_> = (1..=2)
            .zip(next_idx..(next_idx + 2))
            .map(|(t, i)| empty_entry(t, i))
            .collect();
        store.wl().append(&ents).unwrap();
        let mut sm = new_test_raft(1, vec![1, 2, 3], 10, 1, store, &l);
        sm.commit_apply(next_idx - 1);
...
fn test_leader_increase_next() {
    let l = default_logger();
    let next_idx = last_index_if_initialized(2) + 1;
    let previous_ents: Vec<_> = (next_idx..(next_idx + 3))
        .map(|i| empty_entry(1, i))
        .collect();
    let mut tests = vec![
        // state replicate; optimistically increase next
        // previous entries + noop entry + propose
        (
            ProgressState::Replicate,
            2,
            next_idx + previous_ents.len() as u64 + 1 + 1,
        ),
...
fn test_step_ignore_config() {
    ...
    r.step(m).expect("");
    let mut we = empty_entry(2, last_index_if_initialized(2) + 3);
...
fn test_new_leader_pending_config() {
    ...
    let mut tests = vec![
        // `pending_conf_index` will be set to `last_index` when become leader.
        (false, base_idx),
        (true, base_idx + 1),
    ];
...
fn test_leader_transfer_to_slow_follower() {
    ...
    nt.recover();
    assert_eq!(
        nt.peers[&1].prs().get(3).unwrap().matched,
        last_index_if_initialized(3) + 1
    );
...
fn test_leader_transfer_ignore_proposal() {
    ...
    assert_eq!(
        nt.peers[&1].prs().get(1).unwrap().matched,
        last_index_if_initialized(3) + 1
    );
}
...
fn test_batch_msg_append() {
    ...
    for msg in &raft.msgs {
        ...
        assert_eq!(msg.index, base_idx + 1);
    }
