...
impl Ready {
    fn new<T: Storage>(
        ...
        if let Some(e) = rd.entries.first() {
            let conf_states = raft.conf_states();
            if let Some(i) = conf_states.iter().rev().position(|c| c.index < e.index) {
                let s = conf_states.len() - i;
                rd.conf_states = conf_states[s..].to_owned();
            }
        }
