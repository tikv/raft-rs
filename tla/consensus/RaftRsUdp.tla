...
InitNetVars ==
    /\ InitUdpNetworkNetman(Servers, <<"Init", Cardinality(Servers)>>, 
                [n_elec |-> 0, n_ae |-> 0, n_hb |-> 0, n_op |-> 0,  no_inv |-> GetParameterSet("NoInv")])
...
maybe_decr_to(dst, src, m) == 
    ...
    IN  IF progress[dst][src][1] = Replicate
        ...
        ELSE /\ IF \/ nextIndex[dst][src] = 0
                ...
                ELSE LET new_match == Min(rejected, match_hint + 1)
...
HandleMsgAER(m) ==  
    LET resp == m.data
        ...
        next_probe_index == find_conflict_by_term(dst, resp.reject_hint, resp.log_term)    
        failReason ==
...
SC == /\ ScSent /\ ScRecv /\ ScWire  /\ ScOp   /\ ScAe   /\ ScElec
