...
fn test_raw_node_propose_and_conf_change() {
    ...
    loop {
        ...
        if !proposed && rd.ss().is_some() && rd.ss().unwrap().leader_id == raw_node.raft.id {
            ...
            ccdata.reserve_exact(cc.compute_size());
            cc.encode_into(&mut ccdata).unwrap();
...
fn test_raw_node_propose_add_duplicate_node() {
    ...
    let mut propose_conf_change_and_apply = |cc| {
        ...
        for e in rd.committed_entries.as_ref().unwrap() {
            if e.get_entry_type() == EntryType::EntryConfChange {
                let conf_change = ConfChange::decode_from(&e.data).unwrap();
    ...
    let mut ccdata1 = Vec::with_capacity(cc1.compute_size());
    cc1.encode_into(&mut ccdata1).unwrap();
    ...
    let mut ccdata2 = Vec::with_capacity(cc2.compute_size());
    cc2.encode_into(&mut ccdata2).unwrap();
...
fn test_skip_bcast_commit() {
    ...
    cc.node_id = 3;
    let mut data = Vec::with_capacity(cc.compute_size());
    data.reserve_exact(cc.compute_size());
    cc.encode_into(&mut data).unwrap();
