...
impl Scenario {
            ...
            if let Some(entries) = peer.raft_log.next_entries() {
                ...
                for entry in &entries {
                    if entry.get_entry_type() == EntryType::EntryConfChange {
                        let conf_change = ConfChange::decode_from(&entry.data)?;
    ...
    fn assert_membership_change_entry_at<'a>(
        ...
        for peer in peers {
            ...
            let conf_change = ConfChange::decode_from(&entry.data).unwrap();
...
fn begin_entry<'a>(
    ...
    let mut data = Vec::with_capacity(conf_change.compute_size());
    conf_change.encode_into(&mut data).unwrap();
...
fn build_propose_change_message<'a>(
    ...
    message.index = index;
    message.set_entries(vec![begin_entry].into());
    message
}
...
fn build_propose_add_node_message(recipient: u64, added_id: u64, index: u64) -> Message {
    ...
    message.set_msg_type(MessageType::MsgPropose);
    message.index = index;
    message.set_entries(vec![add_nodes_entry].into());
    message
}
